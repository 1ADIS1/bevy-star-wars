...
use crate::LANDSCAPE_SIZE_HALF;

const LASER_SPEED: f32 = 70.;
...
#[derive(Component)]
pub struct GunAwaitingToBeSpawned {
    pub color: Color,
}
...
impl Plugin for LaserPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, (spawn_guns, shoot_guns, move_lasers));
    }
}

fn spawn_guns(
    mut commands: Commands,
    gun_query: Query<(Entity, &GunAwaitingToBeSpawned)>,
    children: Query<&Children>,
    names: Query<&Name>,
) {
    for (entity, spawn_gun) in gun_query.iter() {
        ...
        if spawned_guns {
            commands.entity(entity).remove::<GunAwaitingToBeSpawned>();
        }
    }
}
...
fn move_lasers(
    mut commands: Commands,
    mut laser_query: Query<(&mut Transform, &mut Laser, Entity)>,
    time: Res<Time>,
) {
    for (mut transform, mut laser, entity) in laser_query.iter_mut() {
